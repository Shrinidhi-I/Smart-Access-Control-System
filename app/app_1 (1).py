...
def compare_faces(local_target_image_path, faces_folder, threshold=0.5):
    """
    Compare a target image with a set of face images from a folder.

    Args:
        local_target_image_path (str): Path to the target image.
        faces_folder (str): Folder containing face images.
        threshold (float): Similarity score threshold for determining a match.

    Returns:
        bool: True if a matching face is found, otherwise False.
    """
    # Check if face is detected in the target image
    if not detect_faces(local_target_image_path):
        print("No face detected in the target image. Waiting...")
        return False
    ...
    for face_image in face_images:
        ...
        try:
            ...
            if similarity_score >= threshold:
                # Clean up and return if a match is found
                os.remove(local_face_image_path)
                return True

        except Exception as e:
            print(f"An error occurred while processing {face_image}: {e}")

        finally:
            # Ensure cleanup happens even if there's an error
            if os.path.exists(local_face_image_path):
                os.remove(local_face_image_path)
...
def entry_listener(event):
    if event.data == 1:  # Check if entry is True
        ...
        local_target_image_url = 'http://192.168.190.245/cam-lo.jpg'
        faces_folder = "faces"
